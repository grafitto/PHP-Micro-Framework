...
class Model extends DatabaseObject{
	...
	/**
	 * This returns all includeing trash can
	 * @return array records
	 */
	public static function allIncludeTrash(){
		return self::fetch(NULL,true);
	}
	/*
	 * This gets a database record from the database using column $name
	 * and value $value
	 * @return array $models
	 */
	...
	/**
	 * Gets all notifiations sent to this user
	 */
	public function allNotifications(){
		if(isset($this->id))
			return Notification::get("receiver_id",$this->id);
		return [];
	} 
	public function unreadNotifications(){
		if(isset($this->id)){
			$notifications = Notification::get("read_status", 0);
			$unread = [];
			
			foreach($notifications as $notification){
				if($notification->receiver_id == $this->id && $notification->receiver_type = Session::get('rank')){
					$unread[] = $notification;
				}
			}
			return $unread;
	...
	/**
	 * This returns records in trash can
	 * @return array records
	 */
	public static function getTrash($table = NULL){
		$class = get_called_class();
		if(is_null($table)){
			$table = strtolower($class);
		}
		//create query
		$sql = "SELECT * FROM {$table} WHERE in_trash = 1";
		//Open connection
		self::connect();
